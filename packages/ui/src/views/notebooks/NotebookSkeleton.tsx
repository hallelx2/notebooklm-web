...
function Bar({ className = "" }: { className?: string }) {
 return (
 <span
 className={`relative overflow-hidden bg-accent-soft ${SHIMMER_OVERLAY} ${className}`}
 />
 );
}
...
export function NotebookCardSkeleton() {
 ...
 <div
 aria-hidden="true"
 className="relative bg-surface border border-border-subtle p-5 sm:p-6 flex flex-col gap-4 sm:gap-5 overflow-hidden"
 >
 {/* Title row */}
 ...
 <div className="px-2.5 py-1 rounded-sm flex items-center gap-1.5 bg-border-subtle border border-border-subtle">
 ...
 <div className="flex items-center gap-1.5 px-2.5 py-1.5 border border-border-subtle bg-accent-soft rounded-sm">
 <span className="w-1.5 h-1.5 rounded-full bg-border-strong" />
 <Bar className="block h-2 w-10" />
 </div>
 </div>
...
export function NotebookRowSkeleton() {
 ...
 <div className="w-10 h-10 rounded-md bg-border-subtle border border-border-subtle flex items-center justify-center">
 <span className="material-symbols-outlined text-fg-muted text-lg">
 book_2
 </span>
 </div>
